		...
		<div>
			<form id="frm" action="<c:url value="/search/all?type=${type }" />" class="search-form" method="get">
				...
				<div class="row float-end me-4">
					<select class="form-select form-select-sm col-auto" name="array" aria-label=".form-select-sm example" 
					...
					<input type="hidden" name="keyword" value="${param.keyword }" />
						<ul class="mx-3"style="padding-left: 0px;margin-bottom: 0px;">
							<c:forEach var="BoardDto" items="${noticeList }">
								<li>
									<div class="p-3">
										<h5 id="nTitle" class="fw-bold"><a href="<c:url value="/board/post?article_id=${article_id  }" />">
					...
					<input type="hidden" name="keyword" value="${param.keyword }" />
						<ul class="mx-3"style="padding-left: 0px;margin-bottom: 0px;">
							<c:forEach var="BoardDto" items="${eventList }">
								<li>
									<div class="p-3">
										<h5 id="eTitle" class="fw-bold"><a href="<c:url value="/board/post?article_id=${article_id  }"  />">
												${BoardDto.article_title }</a></h5>
										<p>${BoardDto.article_contents }</p>
											<div class="d-flex flex-row">
												<span class="pe-4"></span>
...
<%-- 				<c:set var="course" value="동아리" /> --%>
				<form action="<c:url value="/search/all?type=${club }" />">
					<input type="hidden" name="type" value="${club }" /> <input type="hidden" name="keyword" value="${param.keyword }" />
						<ul class="mx-3"style="padding-left: 0px;margin-bottom: 0px;">
...
<%-- 				<c:set var="course" value="강좌" /> --%>
				<form action="<c:url value="/search/all?type=${course }" />">
					<input type="hidden" name="type" value="${course }" /> <input type="hidden" name="keyword" value="${param.keyword }" />
						<ul class="mx-3"style="padding-left: 0px;margin-bottom: 0px;">
							<c:forEach var="CourseDto" items="${courseList }">
								<li>
									<div class="p-3">
										<c:set var="course_id" value="${CourseDto.course_id }" />
										<h5 id="cTitle" class="fw-bold"><a href="<c:url value="/course/detail?course_id=${course_id }" />">
